...
class GraphMatrixCompletion(nn.Module):
    def __init__(self, input_dim, side_feat_dim,
        ...
        #kernel_regularizer = regularizers.l2(reg))(input_tensor)
        ...
        self.dense2 = FullyConnected(gcn_hidden_dim + side_hidden_dim, encode_hidden_dim,
                                     dropout=DROPOUT_RATIO, activation=lambda x:x)# 1 /(1+torch.exp(-x)))
    ...
    def reg_loss(self,alpha):
        # 计算权重乘法项
        alpha = alpha
        W1 = self.dense1.weights_u
        W2 = self.dense2.weights_v
        # weightPunish = (alpha / 2) * (tf.reduce_sum(kb.abs(W1.detach().numpy())) + tf.reduce_sum(kb.square(W2.detach().numpy())))
        weightPunish = (alpha / 2) * (tf.reduce_sum(kb.abs(W1.detach().numpy()))+tf.reduce_sum(kb.square(W1.detach().numpy()))+tf.reduce_sum(kb.square(W1.detach().numpy())))
        #w3=(tf.reduce_sum(kb.abs(W1.detach().numpy())) + tf.reduce_sum(kb.abs(W2.detach().numpy()))+tf.reduce_sum(kb.square(W1.detach().numpy()))+tf.reduce_sum(kb.square(W1.detach().numpy())))
        #weightPunish = (alpha / 2) * (tf.reduce_sum(kb.abs(W1.detach().numpy())) + tf.reduce_sum(kb.abs(W2.detach().numpy()))+tf.reduce_sum(kb.square(W1.detach().numpy()))+tf.reduce_sum(kb.square(W1.detach().numpy())))
       # w4=math.sqrt(w3)
        #weightPunish = (alpha / 2)*w4
        return weightPunish
...
#criterion = tf.constant(nn.CrossEntropyLoss()+GraphMatrixCompletion(NODE_INPUT_DIM, SIDE_FEATURE_DIM, GCN_HIDDEN_DIM,
                              #SIDE_HIDDEN_DIM, ENCODE_HIDDEN_DIM, num_basis=NUM_BASIS).reg_loss(0.05)).to(DEVICE)

#reg=0
#for param in FullyConnected.parameters():
    #reg+=0.5*(param ** 2).sum()
#print(reg)
#criterion=criterion( FullyConnected(x),y)+reg_lambda *reg
...
def train():
    ...
    for e in range(EPOCHS):
        ...
        loss= criterion(logits[train_mask], labels[train_mask])+(GraphMatrixCompletion(NODE_INPUT_DIM, SIDE_FEATURE_DIM, GCN_HIDDEN_DIM,
                              SIDE_HIDDEN_DIM, ENCODE_HIDDEN_DIM, num_basis=NUM_BASIS).reg_loss(0.05)).numpy()
       #l1_regularization, l2_regularization=torch.tensor([0],dtype=torch.float32),torch.tensor([0],dtype=torch.float32)
        #loss=loss1+l2_regularization


        rmse = expected_rmse(logits[train_mask], labels[train_mask])+(GraphMatrixCompletion(NODE_INPUT_DIM, SIDE_FEATURE_DIM, GCN_HIDDEN_DIM,
                             SIDE_HIDDEN_DIM, ENCODE_HIDDEN_DIM, num_basis=NUM_BASIS).reg_loss(0.05)).numpy()
        optimizer.zero_grad()
